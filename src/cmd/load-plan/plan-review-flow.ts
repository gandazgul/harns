...
export interface DirectPlanReviewEligibility {
    eligible: boolean;
    reason?: "unsupported_status" | "invalid_execution_policy" | "missing_objective_checks";
    message?: string;
}
...
export function getDirectPlanReviewEligibility(plan: { attrs: PlanFrontMatter }): DirectPlanReviewEligibility {
    ...
    const objectiveChecks = normalizeObjectiveChecks(plan.attrs.objectiveChecks);
    if (!objectiveChecks || objectiveChecks.length === 0) {
        return {
            eligible: false,
            reason: "missing_objective_checks",
            message: "PLANNED_CHANGE Plans need at least one Objective-Failing Check before direct review.",
        };
    }
