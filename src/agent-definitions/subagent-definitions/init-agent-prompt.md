...
- Use `memory` only for project-scoped stores. Do not set `scope: "global"`. Use `memory` with `action: "recall"` to
  learn global preferences, but do not write project facts to global memory.
