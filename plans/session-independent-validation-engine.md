...
`plans/replace-deps-bag-with-capability-ports.md` established the capability-port ownership heuristic (agent turns and
user interactions are genuine boundaries and may be ports; Plan Lifecycle, transitions, registry, and locks are
...
- `src/shared/session/architecture-boundary.test.js` — add a durable rule: any production `validation*.ts` module in
  ...
  fails it.
- `CONTEXT.md` — in the same change, extend the **Workflow Validation** glossary entry (or add one short entry for the
  session-independent validation engine) to state that validation sequencing/convergence is a session-independent engine
  shared by the Core Session runtime, without naming Attached behavior that does not exist yet. No user-visible
  lifecycle or status terms change.
...
- [ ] `CONTEXT.md` describes Workflow Validation as running on a session-independent validation engine (sequencing and
      convergence shared by the Core Session runtime), without claiming Attached behavior that is not implemented yet.
...
- **Open assumptions (reviewable).** (1) Keeping the public `runValidationLoop` signature session-typed, with the port
  ...
  modules directly and implement `ValidationSessionPort` from its own record, which is Epic scope. (2) `CONTEXT.md` gets
  a minimal glossary touch naming the session-independent engine, without describing Attached behavior. (3) No behavior
  change is intended; any deviation the implementation discovers must be raised rather than silently fixed.
