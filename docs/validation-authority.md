...
| --------------------------------------------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| Approved Plan definition                      | Primary `docs/plans/<name>.md` body and reviewable definition fields               | Planner review transition; user-approved Plan Amendment transition                       | Revision-checked Plan write under the Plan lock                                                                     | Execution-worktree Plan copy, browser review, Session prompt        | Retain for Plan lifetime; archive through Plan storage                                             | Load the primary revision; finish a journaled approved amendment before validation              |
| Plan Lifecycle and validation counters        | Primary Plan Front Matter                                                          | `recordPlanEvent` through the lifecycle transition journal                               | Event postcondition written under the Plan lock                                                                     | Session workflow context, TUI/Workspace status, execution Plan copy | Clear attempt-only fields only through their owning lifecycle event                                | Reload primary status; it selects mechanical, semantic, or delivery phase                       |
...
| Objective-Failing Check definition and waiver | Approved Plan definition plus revisioned baseline/waiver fields                    | Planner/user-approved amendment and Objective Check decision transition                  | Revision-checked Plan transition after red-baseline proof or explicit user choice                                   | Engineer repair packet and validation report                        | Replace with a new approved definition; never infer a waiver from an Agent report                  | Reload active checks, baseline, and waivers from the primary Plan                               |
| Objective Check result                        | Typed result from the current mechanical phase                                     | Objective Check runner                                                                   | Mechanical lifecycle event after the complete result is known                                                       | Progress and user message                                           | Discard per-run output after its event; retain approved waivers                                    | Re-run unless the lifecycle event already advanced the Plan                                     |
...
| Mechanical repair result                      | Typed result returned by the isolated repair turn                                  | Current validation owner                                                                 | Isolated Agent turn return; not written to the root Task Completion journal                                         | Progress and immediate fresh CI or Objective Check run              | Discard after the live validation owner reruns checks                                              | No replay after process loss; reclaim the checkpoint and rerun Mechanical Validation            |
...
| Publication and target-ref movement           | Git ref plus transition journal effects; delivery evidence in the Plan after proof | Publication transition only                                                              | Candidate/metadata ancestry proof and compare-and-swap target-ref update                                            | Worktree registry status, Plan delivery summary, UI                 | Retain candidate, worktree, and journal for every non-success result                               | Inspect journal effects and Git ancestry; never replay an uncertain external effect             |
...
- command execution produces a structured CI or Objective Check result;
- Objective Checks distinguish passed, unmet, broken, skipped, and canceled;
...
CI and Objective-Failing Check repairs run in isolated Reviewer-Feedback Engineer turns. When such a turn returns a
structured `task_completed` result to the live validation owner, that owner immediately reruns Mechanical Validation.
The result is not a root Session Task Completion journal entry, so the root Agent Handler cannot claim or acknowledge
it.
...
state, and unresolved transition journals. Those sources must produce one safe next action. If they conflict and no
recorded proof establishes the newer state, RunWield preserves the candidate and fails closed with concrete recovery
actions. It never asks an Agent to edit Front Matter, the worktree registry, a Review Issue, a phase counter, Delivery
Evidence, or a transition journal.
...
- `prevent-objective-check-repair-loops` and the Objective Check amendment flow continue to own broken-check diagnosis,
  baseline proof, and user waiver decisions;
