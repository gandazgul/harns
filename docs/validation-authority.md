...
| Repair completion claim                       | Checkpoint `repairGeneration`, state, receipt, and report                          | Runtime completion boundary after the exact repair Agent calls `task_completed`   | Compare-and-set checkpoint write for that repair generation                                                         | Session continuation and next Reviewer prompt                       | Spend once; retain the report until the Reviewer verifies the findings or the attempt ends         | `ready` resumes Mechanical Validation; `awaiting_repair` resumes the exact repair               |
