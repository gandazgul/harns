...
as project history. RunWield already treats top-level `docs/plans/archived/` as hidden from normal `wld plans` listing
while still allowing explicit file access.
...
RunWield archives Plans by physically moving markdown files from `docs/plans/` to `docs/plans/archived/`, preserving
nested relative paths. For example, `docs/plans/my-epic/01-child.md` archives to
`docs/plans/archived/my-epic/01-child.md`.
...
Normal active workflows stay focused because `wld plans` and active Plan resource APIs hide `docs/plans/archived/`.
