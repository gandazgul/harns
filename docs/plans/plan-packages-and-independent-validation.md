...
complexity: "HIGH"
summary: "Rebuild RunWield planning and delivery around versioned Plan Package folders, a first-class independent Validator, separate semantic review, bounded repair roles, and a durable user-approved defective-Plan return path."
affectedPaths:
    - "docs/plans/"
    - "src/plan-store.js"
...
- a first-class Validator executes every machine-executable and agent-executable step in `validation.md`, remains
  read-only with respect to the candidate and approved package, and submits implementation findings to orchestration;
...
- Each child Planned Change must provide a `validation.md` whose steps are classified as machine-executable,
  agent-executable, or genuinely human-only; the Epic itself is not executed.
