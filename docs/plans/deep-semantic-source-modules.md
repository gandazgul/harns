...
objectiveChecks:
    - id: "OC1"
      command: "test ! -e src/shared && test ! -e src/ui && test ! -e src/cmd && test ! -e src/acp && test ! -e src/tools && test ! -e src/plan-store.js && test ! -e src/plan-front-matter.js && test ! -e src/constants.js && test ! -e src/review-workspace-server.js && test ! -e src/cli.ts"
      rationale: "Every deprecated source root must be gone, not left behind as a forwarding shell. Red today because all of these exist."
    - id: "OC2"
      command: "test \"$(find src/core src/consumers -type f \\( -name '*.ts' -o -name '*.tsx' -o -name '*.js' -o -name '*.jsx' \\) 2>/dev/null | wc -l)\" -gt 400"
      rationale: "599 source files live under the old roots today. This forces the bulk to actually land in the new modules, so a skeleton of empty index.ts placeholders cannot satisfy OC1 and OC3 together."
    - id: "OC3"
      command: "test -f src/core/index.ts && ! grep -rqE '(from|import\\()[[:space:]]*\"[^\"]*(consumers/|design-system/|/tui/|/acp/|/workspace/)' src/core"
      rationale: "No Core file may import a consumer or the design system. Matching the module specifier catches multi-line imports and dynamic import() calls. Verified green on a clean fixture and red on a multi-line violation."
    - id: "OC4"
      command: "test -f src/core/index.ts && ! grep -rqE 'from \"[^\"]*core/[A-Za-z0-9._-]+/' src/consumers src/composition src/design-system src/extensions scripts"
      rationale: "Nothing outside Core may reach past src/core/index.ts into a Core submodule. The trailing slash means core/index.ts does not match while core/sessions/... does."
    - id: "OC5"
      command: "deno run -A scripts/run-tests.js -A --no-check src/core/configuration/themes/theme-carryover.test.ts"
      rationale: "A behavioral test that persists a non-default theme name, resolves it through resolveSelectedThemeJson, renders it through renderRunWieldThemeCss, and asserts the --rw-* values come from that theme. Proves the terminal-to-browser theme carryover survived the theme.js split instead of forking into two merge policies."
    - id: "OC6"
      command: "test -f src/core/collaboration/share-plan.ts && grep -q 'export async function sharePlanForReview' src/core/collaboration/share-plan.ts && ! grep -rq 'commands/plans/share' src/consumers/acp"
      rationale: "The share operation must move into Core with its real declaration, and ACP must stop importing a CLI command. Red today: ACP imports sharePlanForReview from src/cmd/plans/share.ts."
    - id: "OC7"
      command: "! grep -rq 'interactive-session-port' src && ! grep -rq 'SYSTEM_INTERACTIVE_SESSION_PORT' src"
      rationale: "The machinery port that existed only to let commands launch their own TUI must be deleted, not relocated. Red today because both names exist in src/."
