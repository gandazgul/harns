...
objectiveChecks:
    - id: "OC1"
      ...
      rationale: "The mixed-ownership 16-member port must be removed, not renamed or retained beside narrower ports. It exists today."
    - id: "OC2"
      command: "! grep -q 'semanticReviewPort?:' src/shared/workflow/validation.ts src/shared/workflow/validation-session-adapter.ts && ! grep -q 'semanticReviewPort ||' src/shared/workflow/validation.ts src/shared/workflow/validation-session-adapter.ts && ! grep -q 'semanticReviewPort ??' src/shared/workflow/validation.ts src/shared/workflow/validation-session-adapter.ts"
      rationale: "The external Agent-session boundary must be required and explicitly composed. The current public arguments and adapter still permit an optional port with a production fallback."
    - id: "OC3"
      command: "grep -q 'required RunWield machinery port' scripts/check-injection-seams.test.js && deno run -A scripts/run-tests.js scripts/check-injection-seams.test.js"
      rationale: "The detector must reject a required port whose contract exposes product-owned machinery. The current detector deliberately exempts required imported Port types."
    - id: "OC4"
