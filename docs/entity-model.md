...
- **Lifecycle:** Plans participate in Plan Lifecycle. PRDs, ADRs, Work Records, Memories, and Ticket References inform
  planning and navigation but do not participate in Plan Lifecycle.
- **Source-of-truth caveats:** A `Ticket Reference` is a structured URL relation only. RunWield does not copy, own, or
...
| Plan                       | `plans/**/*.md` path and `planId`             | Repository Markdown with YAML Front Matter                       | Plan store and Plan Lifecycle                    | Workspace Plan Board cards are projections over Plan files.                          |
...
| Memory                     | Mnemosyne memory ID/scope                     | Mnemosyne project or global store                                | Memory tools and user/project preferences        | Summaries/preferences; not Plan lifecycle truth.                                     |
| Team Memory                | Canonical repository text plus derived copies | Repository Team Memory text and local Mnemosyne copies           | Trusted Branch and memory reconciliation policy  | Shared retrieval aid; still separate from Plans and Work Records.                    |
