...
| `session/new`    | Required baseline.                              | Validates absolute `cwd`, rejects non-empty `mcpServers` and `additionalDirectories`, requires login plus a usable default model, creates a prompt-ready Runtime session, maps it to an ACP session id, and returns `sessionId` plus `_meta.runwield`. | Rejects required stdio MCP server configurations; returned ACP id is not reliably reloadable through standard `session/load`.                                                |
